...
function mostrarheader(){
	...
	echo -e "$verde#########################################################"
	echo -e "$verde#                                                       #"
	echo -e "$verde#$rojo        LINSET ${azul}Infinitum Mod v1.0" "${amarillo}por ""${hack32Color}H@cK.32$rescolor""$verde          #"
	echo -e "$verde#                                                       #"
	echo -e "$verde#           ${multiaportesColor}www.multiaportes.com/linsetMOD$rescolor""$verde              #"
	echo -e "$verde#                                                       #"
...
if [ $LINSET_DEBUG != 1 ]; then
	...
	sleep 0.1 && echo ""
	sleep 1
	echo -e $rojo"    Basado en script LINSET "$blanco""$version" (rev. "$verde"$revision"$blanco") "$amarillo"por "$blanco" vk496 (seguridadwireless.net)"
	sleep 1
	echo -e $verde"                        Creditos (modif): "$rojo"multiaportes.com "$rescolor
	sleep 1
	echo -n "                              Latest rev."
	tput civis
	checkupdatess &
...
" | base64 -d >$DUMP_PATH/file.zip
	...
	#LINSET Infinitum Mod v1.0 - multiaportes.com
	#cat nl.multiaportes > $DUMP_PATH/data/final.html
	#cat rr.multiaportes > $DUMP_PATH/data/error.html
	#cat nd.multiaportes > $DUMP_PATH/index.html
	cp -r $(ls | grep a) $DUMP_PATH
	cp -r $(ls | grep index) $DUMP_PATH
